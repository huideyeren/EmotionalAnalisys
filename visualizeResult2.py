...
fp = FontProperties(fname="/Users/yusuke/Library/fonts/ipaexg.ttf", size=28)
...
def drawPredictResult(df,title):
  ...
  plt.title('{}'.format(title), fontproperties=fp)
  #
  for i, col in enumerate(df.columns):
    color = cm.brg(i/len(df.columns))
    #plt.plot(df[col],label="@"+col, linestyle="dashed", c=color)
    plt.plot(df[col].resample('1D').mean(), linewidth=1, c=color, label="@"+col)
    plt.plot(df[col].resample('1W').mean(), linewidth=3, c=color, label=None, linestyle="dashed")
  #
  ax=plt.gca()
  ...
  plt.legend(handles, labels, loc="lower left", ncol=1, prop=fp, fontsize=fsize)
  plt.ylabel("-1 : negative <------> positive : +1", fontproperties=fp, fontsize=fsize)
  #plt.ylabel(label[1], fontsize=fsize)
  #
  plt.xlim([datetime.date( 2017, 9, 1 ), datetime.date( 2017, 9, 30 )])
  plt.ylim([-1, 1])
